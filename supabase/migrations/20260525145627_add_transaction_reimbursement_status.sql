alter table public.transactions
  ...
  add column if not exists reimbursed_at date null,
  add column if not exists reimbursement_responsible text null,
  add column if not exists reimbursement_notes text null;

alter table public.transactions
  drop constraint if exists transactions_status_check,
  add constraint transactions_status_check
    check (status in ('ACTIVE', 'REIMBURSED', 'CANCELED'));

create index if not exists transactions_user_status_idx
  on public.transactions (user_id, status);

create index if not exists transactions_user_ignored_in_reports_idx
  on public.transactions (user_id, ignored_in_reports);
