alter table public.financial_audits
  add column if not exists certificate_assertion_id text null,
  add column if not exists certificate_generated_at timestamptz null,
  add column if not exists certificate_verdict text null,
  add column if not exists certificate_agent_id text null;

alter table public.financial_audits
  drop constraint if exists financial_audits_certificate_verdict_check;

alter table public.financial_audits
  add constraint financial_audits_certificate_verdict_check
    check (certificate_verdict is null or certificate_verdict in ('approved', 'rejected'));
...
declare
  ...
  v_payload_auditor_user_id text;
...
begin
  ...
  v_payload_auditor_user_id := nullif(p_certificate_payload->>'auditor_user_id', '');
  ...
  if v_payload_assertion_id is null then
    raise exception 'Campo assertion_id e obrigatorio no certificado.';
  end if;

  if v_payload_auditor_user_id is null or v_payload_auditor_user_id <> v_user_id::text then
    raise exception 'Campo auditor_user_id invalido no certificado.';
  end if;
